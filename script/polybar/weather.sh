...
APIKEY=`secret-tool lookup owm iacobuccivalerio@gmail.com`
CITY_NAME=`curl -s https://ip.city | grep City: | sed "s/<h3>City: //" | sed "s/<\/h3>//"`
COUNTRY_CODE='IT'
...
function outputCompact {
    OUTPUT="%{T$WEATHER_FONT_CODE}%{F$ICON_COLOR}$ICON%{F-}%{T-} $ERR_MSG$COLOR_TEXT_BEGIN$DESCRIPTION$COLOR_TEXT_END$TEMP"
